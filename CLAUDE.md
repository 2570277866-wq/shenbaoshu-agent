...
- [ ] **把 `dify/workflow_v0.8.yml` 导入 Dify 实跑** ← 当前最近的一步
      2026-09-21 已首跑（详见 `docs/开发日志.md` 2026-09-21 条）：
      ✅ 全链路跑通，代码节点 `import re/json` 可用
      ❌ 7 章并行 → 2 个 LLM 节点 300s 读超时 → 已改链式串行（边 20→26）
      ❌ 审查 issues >30 条被 Dify 数组上限拒收 → 已加截断（29+1 汇总）
      提示词加固：04 周期空禁自创、08 数字也要出处
      ⬜ **待复跑**：验证 24300 消失、假出处（张三 S1）是否被第 8 项抓、各节点耗时
...
**已首进 Dify 实跑，抓出两个真问题并已修，等你复跑。**

2026-09-21 首次导入实跑：链路跑通（代码节点可用），但 7 章并行撞单机 Ollama
超时（2 节点红）、审查 issues 超 30 条被 Dify 数组上限拒收 —— 均已修
（链式串行 + 截断，168 单测全绿）。输出审读另发现模型层问题：24300 个月
（周期字段空导致自创 + 数字融合）、假出处引用（张三挂 S1）、无据数字 ——
04/08 提示词已加固。

**下一步：重新导入 `dify/workflow_v0.8.yml` 复跑**，带回：
③ 审查 issues 全文（≤30 条）+ 各节点耗时。重点看 24300 是否消失、
假出处是否被第 8 项抓到。

次要项：`declaration_type` 表单变量没人消费（模板节点未接）；
素材暂由表单逐行录入，接上知识检索后应下线。

单测：`cd scripts && python3 -m unittest discover -s . -p "test_*.py"` → 168 项。
